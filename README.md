...
| -------------- | ------------------------------------------------------------------------------------------ | ---------------------------------------- | ------------------------------------------ |
| Generation     | ``gen, dio, endpoint, corsika, sim, wiremod, ly, offset, spill``                           | Simulates particles hitting the detector | **Heavy**: High CPU, long runtimes   |
| Reconstruction | ``reco1, reco2, digi, track, decode, recluster, fullproduction, ndlar, compress, convert`` | Turns raw signals into 3D tracks/hits    | **Medium**: High memory usage        |
| Merging        | ``merge, skim, hadd, filter, scrub, watchdog, sleep, test, fclless``                       | Combines 100 small files into 1 big file | **Light**: Low CPU, fast, mostly I/O |
...
Jobs with neither an exit code nor an exit signal are removals (`JobStatus == 3`) with a non-empty `RemoveReason`:

1. `RemoveReason` matches a DAG-cascade pattern (removed because `<OtherJobRemoveRequirements..`, or `Node Error: DAG node ...`) —> **Application** (a sibling node's failure removed this job)
2. `RemoveReason` matches `condor_rm (by user..` —> **Submission** (the dominant case; direct user action)
3. `RemoveReason` matches `SYSTEM_PERIODIC_REMOVE...` or another `PeriodicRemove` policy expression (exceeding job limits / Held N days) —> **Submission** (misconfiguration of job requirements)
4. `RemoveReason` matches `JobRouter aborted job` or `JobRouter orphan` —> **Hardware**
5. Any other removal —> Hardware
