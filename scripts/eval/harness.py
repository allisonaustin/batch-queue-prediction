...
from eval.helper import _empty_gpu, _nfeat, _get_slice, log_result, thr_sample, pick_thr, cls_metrics, reg_metrics
...
def run_e1_model(lib, Xm, yv, splits, imp_store, cm_store, ncat):
    ...
    for split, a, b in splits:
        try:
            trs = thr_sample(a)
            spw = float((yva[a] == 0).sum() / max((yva[a] == 1).sum(), 1))
...
def run_e3_model(lib, Xm, failed, hw, splits, imp_store, cm_store, ncat):
    ...
    for split, a, b in splits:
        try:
            ...
            trs = thr_sample(tri)
            spw = float((hw_a[tri] == 0).sum() / max((hw_a[tri] == 1).sum(), 1))
...
if __name__ == "__main__":
    ...
    if args.experiment == "e1":
        ...
        CM = {}
        run_e1_model(
            args.model, Xmatch, failed, SPLITS, IMP, CM, NCAT_MATCH
        )
        ...
        CM = {}
        run_e3_model(
            args.model, Xmatch, failed, hw, SPLITS, IMP, CM, NCAT_MATCH
        )
